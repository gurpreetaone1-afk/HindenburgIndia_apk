...
export const useThemeStore = create<ThemeState>((set, get) => ({
  // Default to "system" so a fresh install follows the phone's OS theme
  // (dark phone → dark app, light phone → light app). Users who explicitly
  // pick Dark/Light in Settings still override this (persisted in MMKV).
  mode: "system",
  ...
  hydrate: () => {
    ...
    const stored = (mmkv.getString(STORAGE_KEYS.theme) ?? "system") as ThemeMode;
    // Unset / unrecognised → "system" (follow the OS). Only an explicit
    // stored "dark"/"light" pins the app to that mode.

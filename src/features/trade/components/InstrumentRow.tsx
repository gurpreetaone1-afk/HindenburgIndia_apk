...
// Return the value only when it's a finite POSITIVE number, else null. Mirrors
// the web's `pickPositive` helper — used to drop the negative bid the admin
// spread can produce on low-priced instruments so we fall back to LTP.
function positiveOr(v: number | null | undefined): number | null {
  const n = Number(v);
...
function InstrumentRowImpl({
  ...
  // Reject non-positive bid/ask before displaying — the admin spread overlay
  // (`_apply_admin_spread`, `bid = ltp - pips/2`) can push the bid NEGATIVE on
  // low-priced crypto (e.g. ADAUSDT ≈ 0.16 with a ~40 absolute spread → bid
  // −19.84). The web guards this the same way (`pickPositive`), so without it
  // the app showed −19.84 while the web showed the LTP fallback. Match web.
  const bid = positiveOr(tick?.bid ?? bidFallback) ?? ltp;
  const ask = positiveOr(tick?.ask ?? askFallback) ?? ltp;

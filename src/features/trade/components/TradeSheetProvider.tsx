  ...
  const value = useMemo<Ctx>(
    () => ({
      open: (args) => {
        ...
        // Remember this instrument so the bottom-nav "Trade" tab (which
        // pushes /(tabs)/trade with no params) reopens the chart on THIS
        // instrument instead of resetting to the NIFTY default.
        useLastInstrumentStore.getState().set(args);
        if (mounted) {

...
export const TradeSheet = forwardRef<TradeSheetRef, TradeSheetProps>(({ initialArgs, onClosed }, ref) => {
  ...
  // Always resolve the intraday (MIS) tier. Product type is automatic — the
  // backend converts an MIS position to carry-forward when the criteria are
  // met; the user does not pick it. The margin tiles still show BOTH tiers
  // via the explicit intraday_* / overnight_* fields below.
  ...
  // stock4x_ind_web/frontend-user/components/trading/OrderPanel.tsx
  ...
  // Resolve the margin tier (leverage / margin% / fixed-per-lot) for one
  // side. The INTRADAY tile reads the explicit `intraday_*` fields and the
  // CARRY-FORWARD tile the `overnight_*` fields, so both stay correct no
  // matter which product_type was fetched — the active `margin_percentage`/
  // `leverage` flip to the overnight tier for NRML-default segments
  // (Infoway forex/crypto), which previously made the intraday tile wrong.
  function marginTier(side: "intraday" | "overnight") {
    const eff = effective.data;
    const pick = (intra?: number | null, ovn?: number | null, active?: number | null) =>
      Number((side === "overnight" ? ovn : intra) ?? active ?? 0);
    const fpm = pick(
      eff?.intraday_fixed_margin_per_lot,
      eff?.overnight_fixed_margin_per_lot,
      eff?.fixed_margin_per_lot,
    );
    const mp = pick(
      eff?.intraday_margin_percentage,
      eff?.overnight_margin_percentage,
      eff?.margin_percentage,
    );
    const lev = pick(eff?.intraday_leverage, eff?.overnight_leverage, eff?.leverage) || 1;
    const mode = (eff?.margin_calc_mode ?? "").toLowerCase();
    return { fpm, mp, lev, mode };
  }
  ...
  function calcRequiredMargin(
    ...
    const { fpm, mp, lev, mode } = marginTier(side);
    if (fpm > 0 && mode === "fixed") return lotCount * fpm;
    if (mp > 0) return (notional * (mp / 100)) / lev;
    if (lev > 1) return notional / lev;
    return 0;
  }

  // Compact "what's being charged" hint for the margin tiles, e.g. "50×",
  // "20%" or "500/lot" — surfaces the leverage the user asked about that
  // the rupee figure alone hides (especially in TIMES mode).
  function marginHint(side: "intraday" | "overnight"): string {
    const eff = effective.data;
    if (!eff) return "";
    const { fpm, mp, lev, mode } = marginTier(side);
    if (mode === "fixed" && fpm > 0) return `${Math.round(fpm)}/lot`;
    if (mode === "percent" && mp > 0) return `${+mp.toFixed(2)}%`;
    if (lev > 1) return `${+lev.toFixed(lev < 10 ? 1 : 0)}×`;
        ...
        {target ? (
          <>
            ...
            <Text tone="muted" size="xs" style={{ marginTop: 4 }}>
              LTP{" "}
              {ltp != null
                ? formatPriceForSegment(ltp, effective.data?.segment_type)
                : "—"}
            </Text>
            ...
            >
              <OutlinePill
                ...
                onPress={openChart}
              />
              {/* Option Chain only for Zerodha F&O-underlying segments
                  (NSE / BSE / MCX). Infoway forex / crypto / commodity
                  instruments have no option chain — the pill is hidden so
                  the user never lands on a blank "no options" page. */}
              {/^(NSE_|BSE_|MCX_)/.test(
                (effective.data?.segment_type ?? "").toUpperCase(),
              ) ? (
                <OutlinePill
                  label="Option Chain"
                  icon="layers-outline"
            ...
            <View style={{ flexDirection: "row", marginTop: 10 }}>
              <KV
                ...
                value={
                  high > 0
                    ? formatPriceForSegment(high, effective.data?.segment_type)
                    : "—"
                }
              />
              <KV
                ...
                value={
                  low > 0
                    ? formatPriceForSegment(low, effective.data?.segment_type)
                    : "—"
                }
              />
              <KV
                ...
                value={
                  open > 0
                    ? formatPriceForSegment(open, effective.data?.segment_type)
                    : "—"
                }
              />
            ...
            >
              {/* Admin-resolved per-instrument lot limits (from
                  /user/segment-settings/effective). Field mapping matches the
                  admin matrix + web OrderPanel: min_lot = min per order,
                  order_lot = max per order, max_each_lot = max per script. */}
              <KV label="Min Lot" value={fmtLot(effective.data?.min_lot)} />
              <KV label="Order Lot" value={fmtLot(effective.data?.order_lot)} />
              <KV label="Max Lots" value={fmtLot(effective.data?.max_each_lot)} />
              <KV label="Lot Size" value={fmtLot(effective.data?.lot_size)} />
            ...
            <View style={{ marginTop: 8, gap: 6 }}>
              <View style={{ flexDirection: "row", gap: 6 }}>
                <KpiCell
                  ...
                  hint={marginHint("intraday")}
                />
                <KpiCell
                  label="CARRY FORWARD"
                  value={compactINR(carryForwardReq)}
                  hint={marginHint("overnight")}
                />
              </View>
              <View style={{ flexDirection: "row", gap: 6 }}>
...
// Admin lot-limit value → display. 0 / null means "no cap / unset" → "—".
// Fractional (forex/crypto min lot like 0.01) keeps 3 decimals; whole lots
// render without decimals.
function fmtLot(v: number | null | undefined): string {
  const n = Number(v ?? 0);
  if (!Number.isFinite(n) || n <= 0) return "—";
  return String(+n.toFixed(n < 1 ? 3 : 0));
}
    ...
    >
      ...
      >
        ...
        {hint ? (
          <Text
            mono
            style={{ fontSize: 9, fontWeight: "800", color: ACCENT_PURPLE }}
          >
            {hint}
          </Text>
        ) : null}

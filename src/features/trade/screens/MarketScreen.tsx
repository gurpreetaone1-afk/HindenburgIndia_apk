...
const BUCKETS: Bucket[] = [
  ...
  { key: "crypto_opt", label: "Crypto OPT", segments: ["CRYPTO_OPTION_BUY", "CRYPTO_OPTION_SELL"], adminRow: "CRYPTO_OPT" },
];
...
function FavList({
  ...
  //
  // The `/user/marketwatch` LIST payload carries each item as
  // `instrument_token` (NOT `token`) — so reading `it.token` alone yielded
  // `String(undefined)` = "undefined" for EVERY favourite, meaning the WS
  // was never subscribed to a single real token and Favorites rows stayed
  // frozen on the 1.5 s REST poll while the order/trade sheet (which
  // subscribes the row's real token itself) ticked live. Mirror the
  // `favTokens` fallback (`token ?? instrument_token`) AND union with the
  // quotes rows' token so the subscribed set exactly matches the token the
  // rows read (`ticks[item.token]`).
  const watchlistTokens = useMemo(() => {
    ...
    for (const r of quotes.data ?? []) {
      if (r.token) set.add(String(r.token));
    }
    return Array.from(set);
  }, [activeWl?.items, quotes.data]);
...
function FeedSegmentList({
  ...
  const { data, isLoading, isError } = useInstrumentSearch({
    q,
    segment: bucket.segments?.join(","),
    // Backend caps this at 100 (`limit: int = Query(le=100)` in
    // api/v1/user/instruments.py). Sending 120 made FastAPI reject the
    // request with HTTP 422 → the list rendered "No instruments found" for
    // EVERY Infoway segment tab (and in-segment search). Keep it <= 100.
    limit: 100,
  });
  const rows = data ?? [];

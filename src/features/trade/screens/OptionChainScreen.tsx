...
// The backend's option-chain resolver (zerodha_service.get_option_chain_fast)
// keys MCX commodity options by Kite's canonical underlying NAME — the exact
// set { GOLD, GOLDM, SILVER, SILVERM, CRUDEOIL, NATURALGAS, COPPER }. But our
// self-seeded MCX futures carry app tradingsymbols like "MCXGOLD" / "MCXCRUDE"
// / "MCXNATGAS". Sending those verbatim made the backend fall through to the
// NFO/BFO catalog, find nothing, and return an empty chain — the "gold ka
// option chain aa hi nahi rha" bug. Map them to the names the backend expects.
// (Live Zerodha MCX futures already resolve via their Kite `name`, e.g.
// "GOLD25AUGFUT" → name "GOLD"; this map only rescues the seeded fallbacks.)
const MCX_OPTION_UNDERLYING: Record<string, string> = {
  MCXGOLD: "GOLD",
  MCXGOLDM: "GOLDM",
  MCXSILVER: "SILVER",
  MCXSILVERM: "SILVERM",
  MCXCRUDE: "CRUDEOIL",
  MCXCRUDEOIL: "CRUDEOIL",
  MCXNATGAS: "NATURALGAS",
  MCXNATURALGAS: "NATURALGAS",
  MCXCOPPER: "COPPER",
};

// Defensive normaliser — strips TradingView-only suffixes so older / stale
// deep-links like "/option-chain/NIFTY1!" still resolve to "NIFTY", and maps
// seeded MCX tradingsymbols to the Kite underlying the backend indexes by.
function normaliseUnderlying(raw: string): string {
  const base = raw
    ...
    .replace(/_50$/, "")
    .trim();
  return MCX_OPTION_UNDERLYING[base] ?? base;
}

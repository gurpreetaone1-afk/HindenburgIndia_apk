...
export function useRemoveMarketwatchItem() {
  ...
  return useMutation({
    mutationFn: ({ id, token }: { id: string; token: string }) => {
      // The backend's default-watchlist delete route keys by the item's
      // MongoDB ObjectId, NOT the instrument token. Sending the raw token
      // made it call PydanticObjectId(token) → InvalidId → HTTP 500
      // ("Server error on items/141496327"). Resolve the item's ObjectId
      // from the cached list and delete by that. Fall back to the token for
      // a just-added optimistic item whose id isn't cached yet (the backend
      // now tolerates both).
      const lists = qc.getQueryData<Marketwatch[]>(KEY);
      const wl = lists?.find((w) => String(w.id) === String(id));
      const item = wl?.items?.find(
        (it) => String(it.token ?? it.instrument_token) === String(token),
      );
      const ref = item?.id ?? token;
      return MarketwatchAPI.removeItem(id, ref);
    },
    onMutate: async ({ id, token }) => {
      await qc.cancelQueries({ queryKey: KEY });
      const snapshot = qc.getQueryData<Marketwatch[]>(KEY);

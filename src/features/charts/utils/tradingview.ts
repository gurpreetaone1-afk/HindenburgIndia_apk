...
export function toTradingViewSymbol(token: string | null | undefined): string {
  ...
  // Crypto SPOT pairs already in Binance form (USDT / USDC / BUSD quote). The
  // Infoway mirror stores them raw — ADAUSDT, BTCUSDT — so map straight to
  // BINANCE. Require a CLEAN pair (letters/digits only, no dashes) so crypto
  // OPTION symbols like "BTC-16JUN26-65000-C-USDT" (which also end in USDT)
  // are NOT matched here — those aren't Binance spot tickers and must fall
  // through to the native /history chart. Checked before the 3-char heuristic
  // whose `length - 3` split mangles 4-char USDT quotes ("ADAUSDT" → "ADAU").
  if (/^[A-Z0-9]{3,12}(USDT|USDC|BUSD)$/.test(sym)) return `BINANCE:${sym}`;

  // Heuristic: 6-letter symbols ending in USD/JPY/EUR/GBP/INR → crypto

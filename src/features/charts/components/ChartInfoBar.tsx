...
function ChartInfoBarImpl({ token, symbol, interval, segment, source }: Props) {
  ...
  // Infoway WS ticks carry only last price — no O/H/L. The REST quote does
  // (backend fills a rolling day bar), so fall back to it for the header's
  // Open / High / Low instead of showing "—".
  const quote = useQuote(token);

  const ltp = tick?.ltp ?? quote.data?.ltp ?? null;
  const open = tick?.open ?? quote.data?.open ?? null;
  const high = tick?.high ?? quote.data?.high ?? null;
  const low = tick?.low ?? quote.data?.low ?? null;
  const close = tick?.close ?? quote.data?.prev_close ?? null;
  ...
  // Forex/crypto need magnitude-aware decimals (AUDUSD 0.65430), not a flat
  // 2 dp that rounded the price to "0.69".
  const fmt = (v: number | null | undefined) =>
    v == null ? "—" : formatPriceForSegment(v, segment);
  const changeDp = priceDecimals(ltp ?? 1);
    ...
    >
      ...
      {change != null || changePct != null ? (
        ...
        >
          ...
          >
            {change != null ? formatSigned(change, changeDp) : ""}
            {changePct != null

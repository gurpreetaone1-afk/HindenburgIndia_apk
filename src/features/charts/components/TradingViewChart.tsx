...
function TradingViewChartImpl({
  ...
  const onMessage = useCallback((e: WebViewMessageEvent) => {
    try {
      ...
      if (msg?.type === "ready") {
        setWebReady(true);
        setChartError(null);
      } else if (msg?.type === "libFailed") {
        setChartError("Chart failed to load. Please reopen the chart.");
      }
    } catch {
      /* ignore non-JSON */
    }
  ...
  const loading = !chartError && (history.isLoading || !webReady);
  const empty = !loading && !chartError && (bars?.length ?? 0) === 0;

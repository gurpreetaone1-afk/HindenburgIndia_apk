...
import { LWC_INLINE_SCRIPT } from "@features/charts/lib/lightweightChartsHtml";

// Lightweight Charts (TradingView, Apache-2.0) is BUNDLED into the app
// (base64 in lib/lightweightCharts.ts) and injected inline — no runtime CDN
// fetch. Previously this loaded from unpkg, which meant any slow/blocked/
// offline network left the chart stuck on an infinite "Loading…" spinner.
// Bundling makes candles render for ANY token the backend /history endpoint
// serves, including options/futures the public TradingView widget can't
// resolve.
...
function NativeChartImpl({ token, interval = "5", displaySymbol }: Props) {
  ...
  function onMessage(e: WebViewMessageEvent) {
    try {
      ...
      } else if (m.type === "libFailed") {
        setLibError("Chart failed to load. Please reopen the chart.");
      }
    } catch {
      /* noop */
    }

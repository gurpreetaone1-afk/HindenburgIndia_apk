...
import { LicensedTVChart } from "@features/charts/components/LicensedTVChart";
...
export function ChartScreen() {
  ...
  // The Trade tab is reached both WITH params (chart button, watchlist,
  // indices) and WITHOUT (bottom-nav "Trade" button). When params are
  // absent, fall back to the last instrument the user engaged with — the
  // one whose order panel was open — so tapping "Trade" keeps that
  // instrument instead of snapping back to NIFTY. Only a cold launch with
  // no history lands on the NIFTY default.
  const lastToken = useLastInstrumentStore((s) => s.token);
  const setLastInstrument = useLastInstrumentStore((s) => s.set);
  const paramToken = params.token ?? params.symbol;
  const token = paramToken ?? lastToken ?? DEFAULT_TOKEN;

  // Keep the "last instrument" in sync when the chart is opened via an
  // explicit param (watchlist/indices/overview don't go through the trade
  // sheet), so a subsequent param-less "Trade" tab tap restores it too.
  useEffect(() => {
    if (paramToken) setLastInstrument({ token: paramToken });
  }, [paramToken, setLastInstrument]);
  ...
  // Crypto OPTION contracts (Bybit: "BTC-16JUN26-65000-C-USDT"). TradingView
  // has no such symbol, so they must chart from our own /history (Bybit
  // candles) via NativeChart — not the free TV widget (which showed "This
  // symbol doesn't exist").
  const isCryptoOption = /-[CP]-USD[TC]?$/i.test(token ?? "");
  ...
  return (
    <Screen padded={false}>
      ...
      <View style={{ flex: 1, minHeight: 0 }}>
        {/* ONE chart engine for everything — the licensed TradingView Charting
            Library (same as the web terminal) in a WebView, fed by the backend
            /history endpoint (which serves Indian/Zerodha, crypto/Binance,
            forex/Yahoo and crypto-options/Bybit alike). Gives indicators,
            drawing tools and fullscreen on every instrument. `token` is passed
            raw so options chart their OWN contract, not the underlying. */}
        <LicensedTVChart token={token} symbol={display} interval={chartInterval} />
      </View>

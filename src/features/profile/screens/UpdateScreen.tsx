...
const VERSION_MANIFEST_URL =
  "https://api.hindenburgindia.live/updates/app-version.json";
...
export function UpdateScreen() {
  ...
  // A native APK update is only "newer" when its build number (versionCode)
  // is strictly higher. Comparing version STRINGS was wrong — it flagged the
  // stale manifest (v0.1.0) as an "update" over the installed v2.0.0.
  const hasNewBuild = !!manifest && manifest.build > me.build;

  const onCheckOta = useCallback(async () => {
    if (checkingOta) return;
    // `checkForUpdateAsync` throws in any build where expo-updates isn't
    // enabled (dev client, Expo Go, or a build without updates configured) —
    // guard first so the user never sees the raw "…has been rejected" error.
    if (!Updates.isEnabled) {
      pushToast({
        kind: "info",
        message: "Auto-updates apply on their own when you reopen the app.",
      });
      return;
    }
    setCheckingOta(true);
    try {
      ...
      // Any failure here is non-fatal — the launch-time auto-check still
      // applies updates. Keep it friendly, not a red error.
      pushToast({
        kind: "info",
        message: "Auto-updates apply when you reopen the app.",
      });
    } finally {
